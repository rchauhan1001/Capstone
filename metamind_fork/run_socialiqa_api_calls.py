"""
Local API runner for MetaMind on Social IQA dataset.
Uses OpenAI API calls (gpt-4o) for local testing.

Usage:
    python run_socialiqa_api_calls.py --max_samples 3
    python run_socialiqa_api_calls.py --start_sample 1 --max_samples 10
"""
...
logger = setup_logger("SocialIQA_API", level=logging.INFO)

SCRATCH = os.environ.get("SCRATCH", f"/scratch/{os.environ.get('USER', 'laredo.ei')}")
LOCAL_DATA_PATH = os.path.join(os.path.dirname(__file__), "socialiqa-train-dev")
...
API_LLM_CONFIG = {
    ...
    "log_path": os.path.join(os.path.dirname(__file__), "results", f"inference_log_api_{time.strftime('%Y%m%d_%H%M%S')}.jsonl"),
}

DEFAULT_DEV_PATH = os.path.join(LOCAL_DATA_PATH, "train.jsonl")
DEFAULT_LABELS_PATH = os.path.join(LOCAL_DATA_PATH, "train-labels.lst")
...
def run(args):
    ...
    results_path = os.path.join(args.output_dir, f"socialiqa_results_api_{timestamp}.jsonl")
    summary_path = os.path.join(args.output_dir, f"summary_api_{timestamp}.json")

    logger.info("Initializing MetaMind pipeline with OpenAI API...")
    llm = OpenAILLM(API_LLM_CONFIG)
    ...
    logger.info(f"Model: {API_LLM_CONFIG['model_name']}")
    logger.info(f"Inference log: {API_LLM_CONFIG['log_path']}")
    ...
    if args.max_samples:
        ...
        logger.info(f"Processing up to {args.max_samples} samples")
    ...
    summary = {
        "timestamp": timestamp,
        "model": API_LLM_CONFIG["model_name"],
        ...
        "inference_log_file": API_LLM_CONFIG["log_path"],
    }
